...
@Component({
  selector: "app-addguestdesigndialog",
  templateUrl: "./addguestdesigndialog.component.html",
  styleUrls: ["./addguestdesigndialog.component.scss"],
})
export class AddguestdesigndialogComponent implements OnInit {
  @ViewChild("jobtype")
  ...
  constructor(public dialogRef: MatDialogRef<AddguestdesigndialogComponent>,
  ...
  getErrorMessage(control: FormControl): string | string | string | string | string | string | string | string | string | string {
  ...
  updateInvalidExpansionPanels(): void {
    const invalidcontrols = this.genericService.findInvalidControls(this.addDesignDialogForm);
  ...
  invertermodelempty(): void {
    if (this.invertermake.value != "Others") {
      ...
      if (this.invertermake.value == 'None') {
        ...
        this.inverterscount.setValidators([Validators.required,
        Validators.min(0),
  ...
  createNewDesignChatGroup(design: Design): void {
    ...
    this.changeDetectorRef.detectChanges();
    this.isLoading = true;
    const GUID = design.chatid.toString();
    ...
    CometChat.createGroup(group).then(
      group => {
        const membersList = [
          ...
          new CometChat.GroupMember("" + 232, CometChat.GROUP_MEMBER_SCOPE.ADMIN)
        ];
  ...
  loginguest(): void {
    this.authenticationService.loginUser(localStorage.getItem('guestemail'), localStorage.getItem('guestpassword')).
      subscribe((response) => {
        this.dialog.closeAll();
