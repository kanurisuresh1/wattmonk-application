        ...
        new CometChat.UserListener({
          onUserOnline: (onlineUser: CometChat.User) => {
            ...
            if (onlineUser.getUid().toString() === this.id.toString()) {
              callback({
                action:
                  CONVERSATION_SCREEN_HEADER_ACTIONS.USER_STATUS_CHANGED.ONLINE,
                payload: { onlineUser },
              });
            }
          },
          onUserOffline: (offlineUser: CometChat.User) => {
            ...
            if (offlineUser.getUid().toString() === this.id.toString()) {
              callback({
                action:
                  CONVERSATION_SCREEN_HEADER_ACTIONS.USER_STATUS_CHANGED
                    .OFFLINE,
                payload: { offlineUser },
              });
            }
      ...
      CometChat.addMessageListener(
        ...
        new CometChat.MessageListener({
          onTypingStarted: (typingIndicator) => {
            callback({
              action:
                CONVERSATION_SCREEN_HEADER_ACTIONS.TYPING_STATUS_CHANGED
                  .TYPING_STARTED,
