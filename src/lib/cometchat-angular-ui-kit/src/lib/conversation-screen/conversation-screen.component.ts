...
export class ConversationScreenComponent {
  @Input() user?;
  @Input() group?;
  @ViewChild("messagecomposer") messagecomposer: MessageComposerComponent;
  ...
  handleEventsFromDetails = (event): void => {
    switch (event.action) {
   ...
   * @param event;
   */
  onMediaMessageSent(event): void {
    switch (event.action) {
      case MEDIA_MESSAGES_COMPOSER_ACTIONS.MEDIA_MESSAGE_SENT: {
        const message = event.payload;
        this.updatedMessagesList = JSON.stringify([message]);
        this.showOptionMenu = false;
        this.messagecomposer.fileOptionsExpanded = false;
        this.cdRef.detectChanges();

        this.actionPerformed.emit({
          action:
            CONVERSATIONS_SCREEN_ACTIONS.MEDIA_MESSAGES_COMPOSER_ACTIONS
              .MEDIA_MESSAGE_SENT,
  ...
  onMessageComposerEvent(event: {
    ...
    switch (event.action) {
      case MESSAGES_COMPOSER_ACTIONS.MESSAGE_SENT: {
        ...
        this.actionPerformed.emit({
          action:
            CONVERSATIONS_SCREEN_ACTIONS.MESSAGES_COMPOSER_ACTIONS.MESSAGE_SENT,
          payload: { message: event.payload },
        });
        break;
      }
      case MESSAGES_COMPOSER_ACTIONS.CLICK_OPTION_MENU: {
        ...
        this.messagecomposer.fileOptionsExpanded = this.showOptionMenu;
        break;
      }
  ...
  optionMenuSelected(event): void {
    ...
    switch (action) {
      case CONVERSATION_SCREEN_HEADER_ACTIONS.AUDIO_CALL_STARTED: {
        if (payload.hasOwnProperty("error")) {
          ...
          this.actionPerformed.emit({
            ...
            payload: { outGoingCall: payload.outGoingCall },
          });
        }
        break;
      }
      ...
      case CONVERSATION_SCREEN_HEADER_ACTIONS.USER_OPTION_MENU_SELECTED: {
        this.genericService.showItemDetails =
          !this.genericService.showItemDetails;
        break;
      }
   ...
   */
  groupMemberActions(event: { action: string; payload?: object | any }): void {
    switch (event.action) {
