...
export class ConversationListManager extends CometChatManager {
  ...
  //     super();
  //     if (searchKey) {
  //         this.conversationRequest = new CometChat.ConversationsRequestBuilder().setLimit(10).setSearchKeyword(searchKey).build();
  //     } else {
  //         this.conversationRequest = new CometChat.ConversationsRequestBuilder().setLimit(10).build();
  //     }
  // }

  attachListener(callback): void {
  ...
  checkAndSendToCallBack(
    message: CometChat.BaseMessage | CometChat.MessageReceipt,
    callback: (
    ...
    if (!isReceipt) {
      ...
      const msgReceipt: CometChat.MessageReceipt =
        message as CometChat.MessageReceipt;
      callback(msgReceipt, true);
    }
  }

  attachUserListener(callback): void {
    ...
    CometChat.addUserListener(
      ...
      new CometChat.UserListener({
        onUserOnline: (onlineUser: CometChat.User) => {
          ...
          callback({
            action: USER_LIST_ACTIONS.USER_STATUS_CHANGED.ONLINE,
            payload: { onlineUser },
          });
        },
        onUserOffline: (offlineUser: CometChat.User) => {
          ...
          callback({
            action: USER_LIST_ACTIONS.USER_STATUS_CHANGED.OFFLINE,
            payload: { offlineUser },
          });
        },
      })
