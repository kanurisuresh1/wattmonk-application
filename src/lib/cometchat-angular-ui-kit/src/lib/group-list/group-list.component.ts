...
export class GroupListComponent implements OnInit {
  ...
  handleGroupCreationActions = (data): void => {
    switch (data.action) {
      case "close_popup":
      ...
      case "group_created":
        if (!data.payload.getIcon()) {
          data.payload.setIcon(
            Helper.getSVGAvatar(
              data.payload.getGuid(),
              "#" + data.payload.getName().substr(0, 1)
            )
          );
        }
  ...
  onItemClick = (group, type?): void => {
    if (this.onItemSelect) {
      this.onItemSelect(group, "group");
    }
    this.actionPerformed.emit({
      action: GROUP_LIST_ACTIONS.GROUP_ITEM_SELECTED,
      payload: { group },
    });
  ...
  onScroll(elem): void {
    if (
      ...
      this.groupListManager.fetchNext().then(
        (groups: CometChat.Group[]) => {
          if (groups.length > 0) {
            groups.map((group, i) => {
              if (!group.getIcon()) {
