...
export class GroupMembersManager extends CometChatManager {
  ...
  constructor(guid, type) {
    ...
    this.guid = guid;
    this.type = type;
    this.groupMembersRequest = new CometChat.GroupMembersRequestBuilder(guid)
      .setLimit(this.limit)
      .build();
  }

  getGroupInfo(guid) {
    return CometChat.getGroup(guid).then(
  ...
  addMember = (
    ...
    role = CometChat.GROUP_MEMBER_SCOPE.PARTICIPANT
  ) => {
    const membersList = [];
