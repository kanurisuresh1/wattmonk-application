...
export class MessageListManager extends CometChatManager {
  ...
  constructor(id, type = "user", private genericService: GenericService) {
    super();
    ...
    if (!isReceipt) {
      ...
      if (msg.getReceiverType() === "user") {
        ...
        if (this.id === msg.getReceiverId()) {
          callback(msg);
          if (msg.getCategory() == "call" && msg.getStatus() == "initiated") {
            this.genericService.playIncomingAudio();
          } else if (
            msg.getCategory() == "call" &&
            (msg.getStatus() == "cancelled" ||
              msg.getStatus() == "unaswered" ||
              msg.getStatus() == "rejected" ||
              msg.getStatus() == "ended" ||
              msg.getStatus() == "ongoing")
          ) {
