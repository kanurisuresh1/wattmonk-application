...
export class SidebarComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(change: SimpleChanges): void {
    let event: { action: string; payload: object | any } =
      change.actionRequired.currentValue;
   ...
   */
  optionMenuSelected(event: { action: string; payload?: object }): void {
  ...
  onItemSelected(event: { action: string; payload?: object | any }): void {
    ...
    if (type === "group") {
      if (!item.hasJoined) {
        const publicGroup: CometChat.Group = new CometChat.Group(
          item.guid,
          item.name,
          item.type
        );
        ...
        CometChat.joinGroup(publicGroup).then(
          (group) => {
            this.actionPerformed.emit({
              action: SIDEBAR_ACTIONS.ITEM_SELECTED,
              payload: { item, type },
            });
          },
          (error) => {
   ...
   * @param event: {action:string, payload:object}
   */
  handleNavigationChange(event): void {
