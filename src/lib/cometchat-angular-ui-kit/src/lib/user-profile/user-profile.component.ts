...
export class UserProfileComponent implements OnInit {
  ...
  onItemClick = ($event, id): void => {
    switch (id) {
      case this.EVENTS[0]:
        this.actionPerformed.emit({
          action: id,
          payload: { user: JSON.parse(this.user) },
        });
        break;
      case this.EVENTS[1]:
        this.actionPerformed.emit({
  ...
  isChatReady = (user?: CometChat.User, error?): void => {
    if (user) {
      if (!user.getAvatar()) {
        user.setAvatar(
          this.getSVGAvatar(user.getUid(), user.getName().substr(0, 1))
        );
      }
  ...
  getSVGAvatar = (generator: string, data: string): string => {
